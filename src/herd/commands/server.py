    ...
    if bound_pid:
        try:
            ...
            if sys.stdin.isatty():
                ...
                kill_it = typer.confirm(
                    "Do you want to forcefully terminate it and take over the port?"
                )
                if kill_it:
                ...
                if is_herd:
                    console.print(
                        f"[yellow]Background mode detected. Auto-killing unresponsive Herd zombie (PID {bound_pid})...[/yellow]"
                    )
                    proc.terminate()
                    proc.wait(timeout=5)
                    time.sleep(1)
                    ...
                    console.print(
                        f"[red]Error: Port {port} is bound by '{proc.name()}' (PID {bound_pid}). Cannot prompt in background mode. Aborting.[/red]"
                    )
                    raise typer.Exit(1)
        except (psutil.NoSuchProcess, psutil.AccessDenied):
            console.print(
                f"[red]Error: Port {port} is in use by an unknown process (Permission denied).[/red]"
            )
            raise typer.Exit(1)

...
class MetricsCollector:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        for model, data in self.stats.items():
            ...
            # Estimate speed based on total duration of requests with completions
            # (only computing speed for completion tokens to measure generation rate)
            avg_speed = c_tokens / data["total_duration_sec"] if data["total_duration_sec"] > 0 and c_tokens > 0 else 0.0
    ...
    def get_prometheus_metrics(self, active_models: list) -> str:
        ...
        lines.append("# HELP herd_model_cpu_percent CPU usage percentage of the active model process.")
        lines.append("# TYPE herd_model_cpu_percent gauge")
        lines.append("# HELP herd_model_memory_bytes RAM usage in bytes of the active model process.")
        ...
        lines.append("# HELP herd_request_duration_seconds_total Total latency spent processing requests in seconds.")
        ...
        for model, data in self.stats.items():
            ...
            # Tokens
            lines.append(f'herd_tokens_total{{model="{model}",type="prompt"}} {data["prompt_tokens_total"]}')
            lines.append(f'herd_tokens_total{{model="{model}",type="completion"}} {data["completion_tokens_total"]}')
            
            # Duration
            lines.append(f'herd_request_duration_seconds_total{{model="{model}"}} {data["total_duration_sec"]:.6f}')
